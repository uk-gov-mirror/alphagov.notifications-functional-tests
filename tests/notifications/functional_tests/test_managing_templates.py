...
def test_copying_an_existing_template(driver, login_seeded_user, template_type, view_class_name):
    ...
    # Create a template
    template_name = f"Functional Tests - copy {template_type} template - {uuid.uuid4()}"
    ...
    choose_existing_template_page.wait_until_url_contains("templates/copy")
    ...
    go_to_templates_page(driver)
    assert templates_page.get_h1_text() == "Templates"
    templates_page.wait_until_url_contains("/templates")
